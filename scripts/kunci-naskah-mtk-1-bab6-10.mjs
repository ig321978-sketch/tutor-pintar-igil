...
function filterCari(judul) {
  ...
  if (teks.includes("nilai tempat") || teks.includes("puluhan dan satuan")) {
    return "materi.ilike.%nilai tempat%,materi.ilike.%puluhan dan satuan%";
  }
  if (teks.includes("penjumlahan dan pengurangan")) {
    return "materi.ilike.%penjumlahan dan pengurangan%20%";
  }
  if (teks.includes("mengukur") || teks.includes("pengukuran")) {
    return "materi.ilike.%mengukur panjang%,materi.ilike.%pengukuran%";
  }
  if (teks.includes("bentuk datar")) {
    return "materi.ilike.%bentuk datar%";
  }
  if (teks.includes("pola gambar") || teks.includes("mengenal diagram")) {
    return "materi.ilike.%pola gambar%,materi.ilike.%mengenal diagram%";
  }
  return `materi.ilike.%${judul}%`;
}
...
function cetakNaskahResmi() {
  const npx = spawnSync(
    ...
    [
      ...
      "--tsconfig",
      "tsconfig.json",
      "scripts/cetak-naskah-mtk-1-bab6-10.ts",
    ],
    {
      cwd: akar,
      encoding: "utf8",
