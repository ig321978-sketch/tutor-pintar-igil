...
function filterCari(judul) {
  const potong = judul
    .replace(/^bab\s+\d+\s*:\s*/i, "")
    .split(",")[0]
    .trim();
  return `materi.ilike.%${potong}%`;
}

function judulAlias(materi) {
  const tambahan = [];
  if (/bilangan cacah sampai 100/i.test(materi)) {
    tambahan.push("Bab 1: Bilangan 1 Sampai dengan 1.000");
    tambahan.push("Bab 1: Bilangan 1 sampai 100");
  }
  if (/penjumlahan dan pengurangan bersusun/i.test(materi)) {
    tambahan.push("Bab 2: Cara Berhitung");
    tambahan.push("Bab 2: Operasi Hitung");
  }
  if (/fondasi perkalian dasar/i.test(materi)) {
    tambahan.push("Bab 3: Waktu dan Durasi");
    tambahan.push("Bab 3: Pengukuran Waktu dan Uang");
  }
  if (/geometri bangun datar/i.test(materi)) {
    tambahan.push("Bab 4: Penjumlahan Bersusun");
    tambahan.push("Bab 4: Pola dan Bangun");
  }
  return tambahan;
}
...
function cetakNaskahResmi() {
  const npx = spawnSync(
    ...
    [
      ...
      "--tsconfig",
      "tsconfig.json",
      "scripts/cetak-naskah-mtk-2-bab1-4.ts",
    ],
    {
      cwd: akar,
      encoding: "utf8",
...
for (const item of daftar) {
  ...
  const alias = [
    KUNCI,
    `${rapikanKunci(KELAS)}|matematika|${rapikanKunci(MATERI)}|${VERSI}`,
    ...judulAlias(MATERI).map(
      (judul) =>
        `${rapikanKunci(KELAS)}|matematika|${rapikanKunci(judul)}|${VERSI}`,
    ),
  ];
  ...
  const cari = await supabase
    ...
    .ilike("kelas", KELAS)
    .ilike("mapel", "%matematika%")
    .or(filterCari(MATERI));
