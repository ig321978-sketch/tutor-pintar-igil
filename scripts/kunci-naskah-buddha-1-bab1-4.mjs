...
function cetakNaskahResmi() {
  const npx = spawnSync(
    ...
    [
      ...
      "--tsconfig",
      "tsconfig.json",
      "scripts/cetak-naskah-buddha-1-bab1-4.ts",
    ],
    {
      cwd: akar,
      encoding: "utf8",
...
for (const item of daftar) {
  ...
  const alias = [
    KUNCI,
    `${rapikanKunci(KELAS)}|pendidikan agama buddha dan budi pekerti|${rapikanKunci(MATERI)}|${VERSI}`,
    `${rapikanKunci(KELAS)}|agama buddha|${rapikanKunci(MATERI)}|${VERSI}`,
  ];
