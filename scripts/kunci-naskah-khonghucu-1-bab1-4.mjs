...
function cetakNaskahResmi() {
  const npx = spawnSync(
    ...
    [
      ...
      "--tsconfig",
      "tsconfig.json",
      "scripts/cetak-naskah-khonghucu-1-bab1-4.ts",
    ],
    {
      cwd: akar,
      encoding: "utf8",
...
for (const item of daftar) {
  ...
  const alias = [
    KUNCI,
    `${rapikanKunci(KELAS)}|pendidikan agama khonghucu dan budi pekerti|${rapikanKunci(MATERI)}|${VERSI}`,
    `${rapikanKunci(KELAS)}|pendidikan agama konghucu dan budi pekerti|${rapikanKunci(MATERI)}|${VERSI}`,
    `${rapikanKunci(KELAS)}|agama khonghucu|${rapikanKunci(MATERI)}|${VERSI}`,
    `${rapikanKunci(KELAS)}|agama konghucu|${rapikanKunci(MATERI)}|${VERSI}`,
  ];
  ...
  const cari = await supabase
    ...
    .ilike("kelas", KELAS)
    .or("mapel.ilike.%khonghucu%,mapel.ilike.%konghucu%")
