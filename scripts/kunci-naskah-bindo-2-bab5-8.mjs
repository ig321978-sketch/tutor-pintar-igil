...
function judulAlias(materi) {
  const tambahan = [];
  if (/bijak lingkungan/i.test(materi)) {
    tambahan.push("Bab 6: Bijak Memakai Uang");
  }
  if (/bijak digital/i.test(materi)) {
    tambahan.push("Bab 7: Sayang Lingkungan");
  }
  if (/cinta budaya nusantara/i.test(materi)) {
    tambahan.push("Bab 8: Hobi yang Jadi Prestasi");
  }
  return tambahan;
}
...
function cetakNaskahResmi() {
  const npx = spawnSync(
    ...
    [
      ...
      "--tsconfig",
      "tsconfig.json",
      "scripts/cetak-naskah-bindo-2-bab5-8.ts",
    ],
    {
      cwd: akar,
      encoding: "utf8",
...
for (const item of daftar) {
  ...
  const alias = [
    KUNCI,
    `${rapikanKunci(KELAS)}|bahasa indonesia|${rapikanKunci(MATERI)}|${VERSI}`,
    ...judulAlias(MATERI).map(
      (judul) =>
        `${rapikanKunci(KELAS)}|bahasa indonesia|${rapikanKunci(judul)}|${VERSI}`,
    ),
  ];
