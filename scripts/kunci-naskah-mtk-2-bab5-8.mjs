...
function judulAlias(materi) {
  const tambahan = [];
  if (/fondasi pembagian dasar/i.test(materi)) {
    tambahan.push("Bab 5: Pengurangan Bersusun");
    tambahan.push("Bab 5: Data Sederhana");
  }
  if (/pengukuran panjang standar/i.test(materi)) {
    tambahan.push("Bab 6: Panjang");
  }
  if (/pengukuran berat standar/i.test(materi)) {
    tambahan.push("Bab 7: Tabel dan Grafik");
  }
  if (/diagram gambar sederhana/i.test(materi)) {
    tambahan.push("Bab 8: Banyaknya Air");
  }
  return tambahan;
}
...
function cetakNaskahResmi() {
  const npx = spawnSync(
    ...
    [
      ...
      "--tsconfig",
      "tsconfig.json",
      "scripts/cetak-naskah-mtk-2-bab5-8.ts",
    ],
    {
      cwd: akar,
      encoding: "utf8",
