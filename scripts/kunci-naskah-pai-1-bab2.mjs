...
const MATERI = "Bab 2: Mengenal Rukun Iman";
const VERSI = "naskah:v3";
const KUNCI = `${rapikanKunci(KELAS)}|${rapikanKunci(MAPEL)}|${rapikanKunci(MATERI)}|${VERSI}`;

function cetakNaskahResmi() {
  const npx = spawnSync(
    "npx",
    ["--yes", "tsx", "--tsconfig", "tsconfig.json", "scripts/cetak-naskah-pai-1-bab2.ts"],
    {
      cwd: akar,
      encoding: "utf8",
      shell: true,
...
const cari = await supabase
  ...
  .ilike("kelas", KELAS)
  .ilike("materi", "%rukun%iman%");

if (cari.error) {
  throw new Error(`Baca cache gagal: ${cari.error.message}`);
}
...
if (upsert.error) {
  throw new Error(`Upsert cache gagal: ${upsert.error.message}`);
}
...
if (cek.error) {
  throw new Error(`Cek cache gagal: ${cek.error.message}`);
}

const utama = cek.data?.[0] ?? null;
if (!utama?.is_locked) {
  throw new Error(
    `Naskah resmi tersimpan tetapi belum terkunci (is_locked=${String(utama?.is_locked)}).`,
  );
}
  ...
  JSON.stringify(
    {
      ...
      is_draft: utama.is_draft,
      model_sumber: utama.model_sumber,
      updated_at: utama.updated_at,
