...
const KELAS = "1 SD";
const MAPEL = "Pendidikan Agama Katolik dan Budi Pekerti";
...
function cetakNaskahResmi() {
  const npx = spawnSync(
    ...
    [
      ...
      "--tsconfig",
      "tsconfig.json",
      "scripts/cetak-naskah-katolik-1-bab1-4.ts",
    ],
    {
      cwd: akar,
      encoding: "utf8",
...
for (const item of daftar) {
  ...
  const alias = [
    KUNCI,
    `${rapikanKunci(KELAS)}|pendidikan agama katolik dan budi pekerti|${rapikanKunci(MATERI)}|${VERSI}`,
    `${rapikanKunci(KELAS)}|agama katolik|${rapikanKunci(MATERI)}|${VERSI}`,
  ];
