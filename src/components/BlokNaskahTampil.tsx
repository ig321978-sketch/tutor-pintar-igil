...
export const DiagramMermaid = memo(function DiagramMermaid({
  sumber,
}: {
  sumber: string;
}) {
...
export function TeksBuku({ teks }: { teks: string }) {
...
const GAYA_KOTAK: Record<GayaKotakBuku, string> = {
  contoh: "border-[#F0AB00] bg-[#FFF8E8] text-slate-800",
  ingat: "border-[#1C01A5] bg-[#EEE9FF] text-slate-800",
  catatan: "border-slate-300 bg-slate-50 text-slate-700",
  ayo: "border-[#1C01A5] bg-white text-slate-800",
};

export function BlokTampil({
  blok,
  padat,
}: {
  ...
  return (
    <div className="overflow-x-auto rounded-2xl border-2 border-[#1C01A5]/15">
        ...
        <tbody>
          {blok.baris.map((baris, indeks) => (
            <tr key={baris.join("-")} className={indeks % 2 ? "bg-slate-50" : "bg-white"}>
              {baris.map((sel, i) => (
                <td key={`${sel}-${i}`} className="px-3 py-2 font-semibold text-slate-700">
                  <TeksBuku teks={sel} />
                </td>
              ))}
            </tr>
